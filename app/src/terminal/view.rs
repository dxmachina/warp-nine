...
use crate::terminal::view::shimmering_loading_text::shimmering_warp_loading_text;
...
impl BlocklistAIRenderContext {
    /// LOCAL FORK: agent conversations went with the agent, so there is never
    /// an active one.
    ...
    pub fn context_color_for_rich_content(
        ...
        // LOCAL FORK: the agent context stripe went with the agent.
        None
    }
    ...
    fn context_color(&self, _theme: &WarpTheme) -> Option<ColorU> {
        // LOCAL FORK: `ai_brand_color` went with the agent.
        None
    }
}
...
struct TerminalViewMouseStates {
    ...
    // LOCAL FORK: parent_conversation_header_link and breadcrumbs_horizontal_scroll
    // went with the conversation breadcrumb row.
}
...
// LOCAL FORK: enum ConversationDetailsPanelAutoOpenPolicy removed with the
// conversation details panel auto-open path.
...
// LOCAL FORK: enum PendingUserQueryKind removed; nothing queues user queries anymore.
...
pub struct TerminalView {
    ...
    // LOCAL FORK: the auto-open bookkeeping (has_auto_opened_conversation_details_panel,
    // conversation_details_panel_auto_open_policy) and the pane-header toggle /
    // ambient-cancel mouse states went with the agent.
...
impl TerminalView {
    ...
    pub fn new(
        ...
        // LOCAL FORK: the ambient agent view model went with the agent.
        ...
        let ai_render_context = Rc::new(RefCell::new(BlocklistAIRenderContext {
            block_ids: HashMap::from_iter([
                (AIContextInclusionState::Pending, Default::default()),
    ...
    fn needs_git_status_for_agent_context(&self, _ctx: &AppContext) -> bool {
        // LOCAL FORK: the agent context that needed git status went with the agent.
        false
    }
    ...
    fn needs_pr_info_for_agent_context(&self, _ctx: &AppContext) -> bool {
        // LOCAL FORK: the agent context that needed PR info went with the agent.
        false
    }
        ...
        if needs_pr_info && self.github_repo_model.is_none() {
            ...
            match result {
                Ok(handle) => {
                    ...
                    let _weak_for_context = handle.downgrade();
    ...
    // LOCAL FORK: fns ambient_agent_view_model, ensure_ambient_agent_view_model,
    // wire_ambient_agent_view_model, suppress_initial_conversation_details_panel_auto_open
    // and its test reader removed with the agent.
    ...
    pub fn is_input_box_visible(&self, model: &TerminalModel, app: &AppContext) -> bool {
        ...
        // LOCAL FORK: the cloud-agent pre-first-exchange check lived in the agent's
        // ambient_agent module; without it the input view is never hidden here.
        ...
        {
            // Show the input if:
            // * The agent is control of the active, long running block.
            // * OR the user has 'tagged in' the agent.
            //
            // LOCAL FORK: the `is_agent_blocked()` guard on the first clause went with
            // the agent's control state. `is_agent_in_control()` is itself a `false`
            // stub now, so dropping the guard cannot change the result.
    ...
    fn should_render_legacy_ambient_agent_loading_footer(
        ...
        // LOCAL FORK: the legacy ambient loading footer needed the agent's
        // ambient_agent module to decide when to show.
        false
    }
    ...
    fn tag_agent_in(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // LOCAL FORK: the input no longer has an agent mode to switch into.
        self.input.update(ctx, |input, ctx| {
    ...
    fn tag_agent_out(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // LOCAL FORK: the input is always in terminal mode now, so there is no mode to
        // switch back to.
    ...
    fn is_any_ai_block_focused(&self, _ctx: &mut ViewContext<Self>) -> bool {
        // LOCAL FORK: AI blocks went with the agent.
        false
    }

    /// LOCAL FORK: the queued prompt inline editor went with the agent, so nothing
    /// inside the input can hold focus this way. Kept so the focus policy above reads
    /// the same.
    fn is_queued_prompt_inline_editor_focused(&self, _ctx: &AppContext) -> bool {
        false
    }
    ...
    fn insert_shell_process_terminated_banner(
        ...
        if self.is_login_shell_bootstrapped {
            ...
            // LOCAL FORK: the shell-termination reason, shell path, antivirus name and OS
            // version were gathered here purely as a telemetry payload. The banner below is
            // the user-visible behaviour and is unaffected.
    ...
    /// LOCAL FORK: both of these handed a prompt to the agent to drive (scaffold a
    /// project, clone a repository). Kept as no-ops so their callers outside this file
    /// keep compiling.
    pub fn create_new_project(&mut self, _prompt: String, _ctx: &mut ViewContext<Self>) {}

    pub fn agent_clone_repository(&mut self, _url: String, _ctx: &mut ViewContext<Self>) {}
            ...
            EnvironmentSetupModeSelectorEvent::Selected(mode) => {
                ...
                match mode {
                    EnvironmentSetupMode::RemoteGitHub => {
                        // LOCAL FORK: opened the environment management pane, which went with
                        // the environments settings page.
                    }
                    EnvironmentSetupMode::LocalRepositories => {
                        // LOCAL FORK: the agent-driven local repo flow went with the agent.
                    }
                }
                self.redetermine_global_focus(ctx);
                ctx.notify();
    ...
    fn agent_view_zero_state_save_position_id(&self, _app: &AppContext) -> Option<String> {
        // LOCAL FORK: the agent view zero state went with the agent.
        None
    }
...
impl TerminalView {
    ...
    // a `change_block_selections` invocation.
        ...
        match block_action {
            ...
            BlockSelectAction::MouseDown(maybe_block_index) => {
                if let Some(block_index) = maybe_block_index {
                    ...
                    // Since rich content blocks cannot be selected, `redetermine_focus` has no
                    // way of knowing whether the user just clicked on a rich content block, so
                    // divert focus away from the input box explicitly.
                    self.focus_terminal(ctx);
                }
            }
    ...
    fn redetermine_global_focus_with_policy(
        ...
        if should_focus_terminal {
            ...
            match self.active_ssh_remote_server_choice_block() {
                ...
                _ => {
                    if self.has_active_init_project(ctx) && self.is_last_block_init_step(ctx) {
    ...
    // LOCAL FORK: fn restore_followup_prompt_after_failed_submission removed with the
    ...
    fn handle_input_event(&mut self, event: &InputEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            InputEvent::ExecuteAIQuery => {
                // LOCAL FORK: the "enter again to send" ephemeral message was an
                // agent view affordance.
            ...
            // * `EnterCloudAgentView` / `ExitCloudModeAndStartLocalAgent` moved between
            //   the cloud agent view and a local agent conversation.
            // Listed explicitly rather than under a wildcard so a variant that is added
            // or renamed still fails the build here.
            InputEvent::AttachDiffSetContext { .. }
            | InputEvent::TryHandlePassiveCodeDiff(_)
            | InputEvent::SubmitCLIAgentInput { .. }
            | InputEvent::EnterCloudAgentView { .. }
    ...
    /// LOCAL FORK: the auth-secret delete confirmation dialog lived on the agent input
    /// and went with the agent. Kept so `pane_group` still compiles; there is never a
    /// dialog to render.
    ...
    /// Sends `text` to the active CLI agent.
    ///
    /// LOCAL FORK: CLI agent sessions went with the agent, so this always returns `None`.
    ...
    fn context_menu_action(&mut self, action: &ContextMenuAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            AskAI(ask_source) => {
                // LOCAL FORK: agent mode went with the agent, so asking it is a no-op.
                let _ = ask_source;
            }
...
impl TypedActionView for TerminalView {
    ...
    fn handle_action(&mut self, action: &TerminalAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            | ExecuteRewindFromInlineMenu { .. } => {
                // LOCAL FORK: AI blocks and conversation rewind went with the agent.
            }
            ...
            // LOCAL FORK: the input has no agent mode to switch into or back out of any
            // more. The actions stay because their keybindings and dispatch sites
            // outside this file still reference them.
            ...
            DeleteAttachment { .. } | OpenAttachmentLightbox { .. } => {
            ...
            InitProject | SummarizeConversation => {
                // LOCAL FORK: /init and /summarize were agent slash commands.
            }
            SetupCloudEnvironment(_) | SetupCloudEnvironmentAndStart(_) => {
                // LOCAL FORK: cloud environment setup went with the agent.
            }
            ...
            OpenViewMCPPane | OpenAddMCPPane => {
                // LOCAL FORK: the MCP settings page went with the agent.
            }
            OpenBillingAndUsagePane => {
            ...
            ToggleLongRunningCommandControl | ToggleHideCliResponses => {
                // LOCAL FORK: the CLI subagent controller went with the agent.
            }
            ExitAgentView => {
                self.exit_agent_view(ctx);
                ctx.notify();
            ...
            EnterCloudAgentView => {
                // LOCAL FORK: cloud agent view went with the agent.
            }
            StartNewAgentConversation { .. } => {}
            OpenInlineHistoryMenu => {
                self.input.update(ctx, |input, ctx| {
            ...
            CancelAmbientAgentTask | ToggleUsageFooter => {
                // LOCAL FORK: ambient agent tasks and the usage footer went with the agent.
            }
            RevealChildAgent { .. }
            | SwitchAgentViewToConversation { .. }
            | OpenChildAgentInNewPane { .. }
            ...
            | CyclePreviousOrchestrationChildAgent
            | CycleNextOrchestrationChildAgent => {
                // LOCAL FORK: agent orchestration went with the agent.
            }
            ToggleSessionRecording => {
...
impl View for TerminalView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
            ...
            (input_mode, _, _) => {
                if self.input.as_ref(app).is_cloud_mode_input_v2_composing(app) {
                    ...
                    if input_box_visible {
                        ...
                        // LOCAL FORK: the ambient loading footer went with the agent.
        ...
        // LOCAL FORK: the conversation details panel went with the agent, so the pane is
        // never wrapped.
        final_element
    }
    ...
    fn keymap_context(&self, app: &AppContext) -> warpui::keymap::Context {
        ...
        if active_block.is_active_and_long_running() {
            ...
            if active_block.is_agent_monitoring() {
                ...
                // LOCAL FORK: `Block::is_eligible_for_agent_handoff` went with the
                // agent, so the user-took-over context key is never set. The key
                // itself stays; `view/init.rs` still binds against it.
            }
        }
...
impl Drop for TerminalView {
    fn drop(&mut self) {
            ...
            && (has_pending_ssh_session || !is_bootstrapped)
        {
            ...
            // LOCAL FORK: the abandoned-session report went with telemetry. The log line
            // above it is kept, so the condition is still observable in the log.
        };
    }
}
