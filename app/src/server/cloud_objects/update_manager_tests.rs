...
// LOCAL FORK: the tests of the cloud object fetch response went with it. They replayed
// an `InitialLoadResponse` through `on_changed_objects_fetched` and asserted the model
// and sqlite writes that followed.
...
// LOCAL FORK: `test_empty_trash` and `test_leave_shared_object` went with the
// online-only operations they drove.
...
// LOCAL FORK: the ten `test_move_object_*` tests went with the online-only move
// operation. They asserted the outbound MoveObject/MoveToDrive requests and the
// optimistic-update revert on failure. The three `*_over_rtc` tests below stay:
// they cover applying a move that arrives as an object update, not sending one.
