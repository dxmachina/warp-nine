...
// LOCAL FORK: the conversation transcript details panel (`ConversationDetailsPanel`),
// the info button that toggled it, the "View all cloud runs" button and the
// `should_show_conversation_details_panel` helper all went with the agent. They were
// only reachable from ambient-agent tasks and AI conversation history, both of which
// no longer exist.
