...
fn main() -> Result<()> {
    ...
    if target_os == "macos" && target_family != "wasm" {
        ...
        // LOCAL FORK: the dock tile plugin is no longer built.
        //
        // It existed only to swap the Dock icon for one of several alternates and
        // to keep that icon while the app was not running. This fork ships a
        // single icon, and the plugin is no longer copied into the bundle by
        // `script/macos/bundle`, so building it here was pure cost: a `make`
        // shellout to clang on every build of this crate. The sources are kept in
