...
//!
//! LOCAL FORK: lifted verbatim out of `crates/ai/src/diff_validation/`. It lived in
//! the agent crate because the agent produced diffs, but it has zero agent
//! dependency: its imports are `std`, `itertools`, `regex`, `serde`, `strsim`,
//! `lazy_static` and `derivative`. The code viewer, inline diff and code review all
//! consume it, so it comes across rather than going out.
