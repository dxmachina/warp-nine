...
// LOCAL FORK: the pure secret-*detection* functions moved to
// `terminal/model/secret_detection.rs` — they are used by workflows, notebooks
// and the MCP settings editor, none of which are agent code. What remains here
// is the redaction rendering, which is written against `AIAgentOutput`.
use crate::terminal::model::secret_detection::{
    SECRET_REDACTION_REPLACEMENT_CHARACTER, find_secrets_in_text_with_levels,
    find_secrets_in_text_with_levels_using_regex,
};
