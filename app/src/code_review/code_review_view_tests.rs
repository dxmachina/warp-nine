...
/// Applies `edits` to a mirror of `previous`, pulling each inserted path from
/// `new` at the index it is inserted at — the same contract
/// `invalidate_all` relies on when it rebuilds the file map alongside the rows.
fn apply_row_edits(previous: &[String], new: &[String], edits: &[RowEdit]) -> Vec<String> {
    let mut rows = previous.to_vec();
    for edit in edits {
        match *edit {
            RowEdit::Remove(index) => {
                rows.remove(index);
            }
            RowEdit::Insert(index) => rows.insert(index, new[index].clone()),
        }
    }
    rows
}
...
#[test]
fn test_list_row_edits_are_empty_when_nothing_moved() {
    let order = paths(&["a.rs", "b.rs", "c.rs"]);
    assert!(list_row_edits(&order, &order).is_empty());
}

#[test]
fn test_list_row_edits_only_touch_rows_that_changed() {
    let previous = paths(&["a.rs", "b.rs", "c.rs"]);
    let new = paths(&["a.rs", "b.rs", "c.rs", "d.rs"]);

    // Appending a file must not disturb the rows already on screen, otherwise
    // their measured heights (and the scroll anchor) are thrown away.
    assert_eq!(list_row_edits(&previous, &new), vec![RowEdit::Insert(3)]);

    let removed = paths(&["a.rs", "c.rs"]);
    assert_eq!(
        list_row_edits(&previous, &removed),
        vec![RowEdit::Remove(1)]
    );
}

#[test]
fn test_list_row_edits_insert_new_file_in_place() {
    let previous = paths(&["a.rs", "c.rs"]);
    let new = paths(&["a.rs", "b.rs", "c.rs"]);

    let edits = list_row_edits(&previous, &new);
    assert_eq!(edits, vec![RowEdit::Insert(1)]);
    assert_eq!(apply_row_edits(&previous, &new, &edits), new);
}

#[test]
fn test_list_row_edits_handle_a_moved_file() {
    // A file leaving the untracked group after `git add` moves up the list.
    let previous = paths(&["a.rs", "c.rs", "b.rs"]);
    let new = paths(&["a.rs", "b.rs", "c.rs"]);
...
#[test]
fn test_list_row_edits_handle_simultaneous_add_remove_and_reorder() {
    let previous = paths(&["a.rs", "b.rs", "c.rs", "d.rs"]);
    let new = paths(&["c.rs", "a.rs", "e.rs"]);
...
#[test]
fn test_list_row_edits_from_empty_and_to_empty() {
    let new = paths(&["a.rs", "b.rs"]);
    let edits = list_row_edits(&[], &new);
    assert_eq!(edits, vec![RowEdit::Insert(0), RowEdit::Insert(1)]);
    assert_eq!(apply_row_edits(&[], &new, &edits), new);

    let edits = list_row_edits(&new, &[]);
    assert_eq!(edits, vec![RowEdit::Remove(1), RowEdit::Remove(0)]);
    assert!(apply_row_edits(&new, &[], &edits).is_empty());
}

#[test]
fn test_file_order_key_puts_untracked_files_last() {
    let tracked = FileDiff {
    ...
    // Matches `git status --porcelain=2`: tracked changes first, then
    // untracked, each ordered by path — not one flat lexicographic sort.
    assert!(
        CodeReviewView::file_order_key(&tracked) < CodeReviewView::file_order_key(&untracked),
        "a tracked file sorts before an untracked one regardless of path"
    );
}
