...
pub fn menu_bar(ctx: &mut AppContext) -> MenuBar {
    MenuBar::new(vec![
        ...
        // LOCAL FORK: the "AI" and "Drive" top-level menus are removed. Their
        // builders (`make_new_ai_menu` / `make_new_drive_menu`) are left in place
        // for now so the CustomActions they reference stay wired until the agent
        // and Drive subsystems are deleted outright.
...
// LOCAL FORK: `make_new_ai_menu` removed. It built the top-level "AI" menu from
// CustomAction::{NewAgentModePane, AttachSelectionAsAgentModeContext, AISearch,
// OpenAIFactCollection, OpenMCPServerCollection}.
...
// LOCAL FORK: `make_new_drive_menu` removed. It built the top-level "Drive" menu
// (personal/team workflows, notebooks, AI prompts, env vars, Warp Drive toggle,
// team settings, AI fact collection, MCP servers, and session sharing).
