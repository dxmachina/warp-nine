...
impl<S> TerminalManager<S> {
        ...
        BoxManager: FnOnce(Self) -> Box<dyn TerminalManagerTrait> + 'static,
    {
        ...
        // LOCAL FORK: a session created from a "share this" entry point used to be put
        // into `SharePendingPreBootstrap` here, before the surface existed, so the
        // bootstrap handler could pick it up and start the share.
