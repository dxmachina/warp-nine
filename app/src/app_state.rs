...
pub enum LeafContents {
    ...
    // LOCAL FORK: `EnvironmentManagement(EnvironmentManagementPaneSnapshot)` went with the
    // cloud Environments settings page. It was never persisted (`is_persisted` returned
    // false for it), so no stored pane rows reference it.
