...
impl TerminalModel {
    ...
    pub fn is_cloud_agent_conversation(&self) -> bool {
        // LOCAL FORK: the two things that made this true were the
        // `ViewingAmbientConversation` transcript-viewer status, which went with the
        // agent, and a shared ambient session, which went with session sharing.
        false
    }
    ...
    pub fn resize(&mut self, size_update: SizeUpdate) {
            ...
            || size_update.rows_or_columns_changed()
        {
            ...
            // LOCAL FORK: the two reasons not to reflow old blocks on resize were both
            // shared-session ones (a viewer being told the sharer's size, a sharer honoring
            // a viewer's reported size). Every remaining resize is a genuine local pane or
            // font change, which should reflow.
            self.block_list
...
impl ansi::Handler for TerminalModel {
    ...
    fn should_validate_dcs_hook_session_id(&self) -> bool {
        // LOCAL FORK: only a shared-session viewer skipped this check, because the session
        // ids in a replayed scrollback belong to the sharer's shell, not ours.
        true
    }
