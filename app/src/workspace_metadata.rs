//! Workspace metadata persisted in SQLite.
//!
//! LOCAL FORK: lifted verbatim out of `crates/ai/src/workspace.rs`. Despite living
//! in the agent crate it is plain data with no agent dependency: `chrono`,
//! `std::path` and `persistence::model`. Moving it here is what lets the `ai`
//! crate be deleted.
