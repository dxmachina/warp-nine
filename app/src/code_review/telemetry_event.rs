//! LOCAL FORK: this file was the code review telemetry event enum. All of it went
//! with telemetry except `CodeReviewPaneEntrypoint`, which is carried as a payload on
//! `WorkspaceAction`/`TerminalViewAction` variants and on `OpenCodeReviewPaneParams`.
//! The module path is unchanged so its importers do not move.
