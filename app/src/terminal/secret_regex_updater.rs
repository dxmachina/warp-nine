...
impl CustomSecretRegexUpdater {
    ...
    fn update_custom_secret_regex_list(&self, ctx: &mut ModelContext<Self>) {
        ...
        // LOCAL FORK: the second pass here fed the telemetry-side secret regex in
        // `server/telemetry/secret_redaction.rs`, which went with telemetry. The
        // terminal-side regexes above, which drive visual obfuscation, are unchanged.
    }
}
