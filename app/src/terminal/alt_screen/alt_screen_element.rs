...
impl AltScreenElement {
    ...
    fn on_scroll(
        ...
        // LOCAL FORK: the alt screen was vertically scrollable only for a shared-session
        // reader whose window was smaller than the sharer's.
...
impl ScrollableElement for AltScreenElement {
    ...
    fn scroll(&mut self, delta: Pixels, ctx: &mut EventContext) {
        ...
        // LOCAL FORK: the scroll top was mirrored back to the terminal view so a
        // shared-session viewer's alt-screen scroll survived a re-render.
        let _ = ctx;
    }
}
