...
1. **First compile on a real Windows machine.** The agent excision was only
   ever validated against the macOS targets ("1,781 errors -> 0" was lib +
   warp-nine bin on mac). Name-level greps are clean, but greps cannot catch
   changed *signatures* -- a `cfg(windows)` call site passing an argument that
   left with telemetry would only surface under `cargo check`. Start with:
   `.\script\windows\bundle.ps1 -CHANNEL oss -check-only`
   Expect a handful of errors of the same repair character as the mac pass,
   not another 1,781.
2. **The PowerShell cmdlet completion specs are still dropped.** The 663 specs
   in `crates/warp_command_signatures/json/autogenerated/powershell` (6.3 MB)
   were deleted on the rationale "unreachable on macOS" -- which stops holding
   the moment this runs on Windows, where pwsh is the primary shell. PowerShell
   *sessions* work fine (the pwsh bootstrap and ConPTY plumbing are untouched);
   only cmdlet completions are gone. Restoring means re-vendoring
   `json/autogenerated/powershell` from warpdotdev/command-signatures @
   29cd61c3 and re-adding the `PowershellAutogeneratedAssets` struct in
   `src/assets.rs`. Decide after the build is green.
3. `app/src/autoupdate/windows.rs` compiles against surviving modules but its
   tests went with the telemetry sweep, and it still polls the upstream release
   asset URL for updates that will not exist for WarpNine. Stub it or point it
   at this fork's GitHub releases before shipping anything.
