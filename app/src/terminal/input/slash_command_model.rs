...
impl SlashCommandModel {
    pub fn new(
        ...
        // LOCAL FORK: the old modality gated slash commands on the agent input
        // model's lock state. That model went with the agent.
    ...
    pub fn disable(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // LOCAL FORK: dismissing the menu used to flip the agent input model back
        // to shell mode. There is no agent input mode to flip.
