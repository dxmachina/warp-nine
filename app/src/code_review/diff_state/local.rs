...
impl LocalDiffStateModel {
    ...
    pub fn git_commit_chain(
        ...
        branch: String,
        // LOCAL FORK: AI PR title/body generation went out with the agent, so the
        // flag no longer selects anything; `gh pr create` fills the content.
    ...
    pub fn create_pr(
        &self,
        branch: String,
        // LOCAL FORK: AI PR content generation went out with the agent.
    ...
    /// LOCAL FORK: AI commit-message generation went out with the agent
    /// (`git_actions::generate_commit_message` was an LLM call). The method is
    /// kept so `DiffStateModel::generate_commit_message` still dispatches, and
    /// reports failure immediately; `git_dialog::commit` already handles the
    /// `Err` arm by falling back to the plain commit-message placeholder.
    pub fn generate_commit_message(
        ...
        ctx.emit(DiffStateModelEvent::CommitMessageGenerated(Err(
            "commit message generation is not available in this build".to_string(),
        )));
    }
