...
impl CloudViewModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        // LOCAL FORK: this model no longer listens to the UpdateManager. Its handler ran
        // only on `ObjectOperation::Create` results, to drop the folder-timestamp cache
        // entry keyed by a client id once the server replaced that id, and to invalidate
        // the new object's ancestors' sort timestamps. Objects keep their client id now,
        // so nothing is rekeyed; ancestor invalidation already happens on the CloudModel
        // events this model still subscribes to.
        Self {
