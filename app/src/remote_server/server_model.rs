...
use super::proto::{
    ...
    CodebaseIndexStatus, CodebaseIndexStatusUpdated, CodebaseIndexStatusesSnapshot, DeleteFile,
    DeleteFileResponse, DeleteFileSuccess, DiscardFilesError, DiscardFilesResponse,
    DiscardFilesSuccess, ErrorCode, ErrorResponse, FileOperationError,
    ...
    GitCreatePrResponse, GitGetCommittedBranchFilesRequest, GitGetCommittedBranchFilesResponse,
...
// LOCAL FORK: `CodebaseIndexRequest`, `CodebaseIndexRequestParams` and
// `CodebaseIndexRequestPathKind` only prepared requests for the agent's embedding index
// manager; they went with it.
...
impl ServerModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        // LOCAL FORK: the codebase index manager subscription went with the agent.
            ...
            Some(client_message::Message::HostScoped(wrapper)) => {
                let outcome = match wrapper.message {
                    ...
                    Some(host_scoped_request::Message::IndexCodebase(m)) => {
                        Self::codebase_indexing_removed_response(
                            "IndexCodebase",
                            &request_id,
                            conn_id,
                            not_enabled_codebase_index_status(m.repo_path),
                        )
                    }
                    Some(host_scoped_request::Message::DropCodebaseIndex(m)) => {
                        Self::codebase_indexing_removed_response(
                            ...
                            &request_id,
                            conn_id,
                            disabled_codebase_index_status(m.repo_path),
                        )
                    }
                    Some(host_scoped_request::Message::GetFragmentMetadataFromHash(m)) => {
                        self.handle_get_fragment_metadata_from_hash(m, &request_id, conn_id)
                    }
                    ...
                    Some(host_scoped_request::Message::ResyncCodebase(m)) => {
                        let status = unavailable_codebase_index_status(
                            m.repo_path,
    ...
    // LOCAL FORK: `handle_codebase_index_manager_event`, `push_codebase_index_status`,
    // `push_codebase_index_status_update` and `codebase_index_status` all read the agent's
    // embedding index manager and went with it. The daemon still answers the bootstrap
    // snapshot so a connecting client gets a defined (empty) view.

    fn push_codebase_index_statuses_snapshot(&mut self, conn_id: ConnectionId) {
        ...
        log::debug!(
            "[Remote codebase indexing] Daemon pushing empty bootstrap codebase index statuses snapshot: conn_id={conn_id}"
        );
        self.send_server_message(
            Some(conn_id),
            None,
    ...
    /// LOCAL FORK: `IndexCodebase`, `ResyncCodebase` and `DropCodebaseIndex` were served by
    /// the agent's embedding index manager. The wire protocol is unchanged, so the daemon
    /// still answers each request with a fixed status instead of dropping the message.
    fn codebase_indexing_removed_response(
        operation_name: &str,
        request_id: &RequestId,
        conn_id: ConnectionId,
        ...
        log::info!(
            "[Remote codebase indexing] Daemon rejecting {operation_name} because local codebase indexing was removed: request_id={request_id} conn_id={conn_id}"
        );
        codebase_index_status_response(status)
    }

    // LOCAL FORK: `handle_index_codebase`, `handle_resync_codebase`,
    // `current_codebase_index_status_or_queued` and `handle_drop_codebase_index` drove the
    // agent's embedding index manager. Their dispatch arms now answer with a fixed status via
    // `codebase_indexing_removed_response`.

    /// LOCAL FORK: fragment metadata came out of the agent's embedding index, which went
    /// with the agent. The request is still answered so the client sees a defined protocol
    /// error rather than a timeout; `validate_fragment_metadata_lookup` and the
    /// hash-parsing/lookup body went with the index manager.
    fn handle_get_fragment_metadata_from_hash(
        ...
        log::info!(
            "[Remote codebase indexing] Daemon rejecting GetFragmentMetadataFromHash because \
             local codebase indexing was removed: request_id={request_id} conn_id={conn_id} \
             repo_path={}",
            msg.repo_path
        );
    ...
    fn handle_initialize(
        ...
        // LOCAL FORK: the client-supplied codebase index limits and the persisted index
        // restore both fed the agent's embedding index manager and went with it.
    ...
    fn handle_update_preferences(
        ...
        // LOCAL FORK: the codebase index limits carried by `UpdatePreferences` only tuned the
        // agent's embedding index manager and went with it.
        #[cfg(feature = "crash_reporting")]
        {
            if msg.crash_reporting_enabled {
    ...
    // LOCAL FORK: `validate_remote_codebase_index_auth` and `prepare_codebase_index_request`
    // only guarded requests bound for the agent's embedding index manager; they went with it.
...
// LOCAL FORK: `canonicalize_index_repo_path` and `missing_fragment_metadata` only served
// the embedding-index request handlers and went with them.
...
// LOCAL FORK: `fragment_metadata_lookup_error_response_from_error` mapped the index
// manager's lookup error, and `fragment_metadata_to_proto` projected its fragment metadata
// onto the wire type. Both went with the agent.
