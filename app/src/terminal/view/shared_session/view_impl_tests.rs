...
/// LOCAL FORK: this was `test_on_session_share_ended_skips_cloud_continuation_for_user_share_with_task_id`.
/// The cloud-continuation half went with the agent; what remains is the shared-session
/// guarantee it also pinned: ending a share on a non-viewer inserts exactly one ended
/// banner and leaves the input box visible and editable.
#[test]
fn test_on_session_share_ended_keeps_input_editable_for_user_share() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
