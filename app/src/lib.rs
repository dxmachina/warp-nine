...
pub(crate) enum LaunchMode {
    ...
    // LOCAL FORK: LaunchMode::CommandLine ran the Warp command-line SDK, whose
    // subcommands were all agent and cloud surfaces. It went with `warp_cli`'s
    // `CliCommand` tree.
...
pub fn run() -> Result<()> {
    ...
    // If running as a standalone CLI binary, print help instead of launching the
    // GUI app.
    //
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    app_builder.run(move |ctx| {
        ...
        // LOCAL FORK: the TUI front-end used to branch off here, reusing the
        // `initialize_app` bootstrap above and mounting itself instead of calling
        // `launch()`. It went with the `warp_tui` crate, so every launch mode now
        // takes the GUI path.
...
pub(crate) fn initialize_app(
    ...
    // LOCAL FORK: these were registered by `ai::init`, but the widgets themselves
    // are generic and outlived the agent.
    ui_components::keyboard_navigable_buttons::init(ctx);
    ui_components::toggleable_items::init(ctx);
    ...
    // LOCAL FORK: the MCP, skill, AI document and agent conversation singletons
    // were registered here; they belong to the removed agent subsystem.
    ...
    // LOCAL FORK: the cloud-object `Listener` went with cloud sync. It held the realtime
    // websocket subscription that pushed other devices' object edits into `CloudModel`.
    ...
    // LOCAL FORK: the scheduled agent manager, LLM preference/harness singletons
    // and the agent tip model were registered here; all belonged to the agent.
